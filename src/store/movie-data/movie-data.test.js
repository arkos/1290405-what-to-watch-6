...
describe(`Reducer 'movieData should work correctly'`, () => {
  it(`Reducer without additional parameters should return initial state`, () => {
    expect(movieData(undefined, {})).toEqual({
      movies: [],
      isDataLoaded: false,
    });
  });
  ...
  it(`Reducer should update movies by loadMovies`, () => {
    ...
    expect(movieData(state, loadMovies(movies))).toEqual({
      movies,
      isDataLoaded: true,
    });
  });
  ...
  it(`Reducer should update reviews by loadReviews`, () => {
    ...
    expect(
      movieData(state, loadReviews(sampleReviews, sampleMovie.id))
    ).toEqual({
      movies: expect.arrayContaining([
        expect.objectContaining({ reviews: sampleReviews }),
      ]),
      isDataLoaded: true,
    });
  });
...
it(`Reducer should update reviews by loadReviews even if there are no reviews`, () => {
  ...
  expect(movieData(state, loadReviews(sampleReviews, sampleMovie.id))).toEqual({
    movies: expect.arrayContaining([
      expect.objectContaining({ reviews: sampleReviews }),
    ]),
    isDataLoaded: true,
...
it(`Reducer should not update movie by reloadMovie if it's null`, () => {
  ...
  expect(movieData(state, reloadMovie(null))).toEqual({
    movies: expect.not.arrayContaining([null]),
    isDataLoaded: true,
  });
});
...
it(`Reducer should update movie by reloadMovie if it's not null`, () => {
  ...
  expect(movieData(state, reloadMovie(expectedMovie))).toEqual({
    movies: expect.arrayContaining([expectedMovie]),
    isDataLoaded: true,
  });
});
...
describe(`Async operation should work correctly`, () => {
  it(`Should make a correct API call to /films`, () => {
    ...
    apiMock.onGet(APIRoute.MOVIES).reply(200, [{ fake: true }]);

    return movieLoader(dispatch, () => {}, api).then(() => {
      expect(dispatch).toHaveBeenCalledTimes(2);
      expect(dispatch).toHaveBeenNthCalledWith(1, loadMovies([{ fake: true }]));
    });
  });
  ...
  it(`Should make a correct API call to /films/promo`, () => {
    ...
    apiMock.onGet(getPromoMovieUrl()).reply(200, { fake: true });

    return promoLoader(dispatch, () => {}, api).then(() => {
      expect(dispatch).toHaveBeenCalledTimes(1);
      expect(dispatch).toHaveBeenNthCalledWith(1, loadPromo({ fake: true }));
    });
  });
});
